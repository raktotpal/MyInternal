...
public class ParquetUtility {
	...
	public static void convertCsvToParquet(Map<String, String> schemaInfo,
			File csvFile, File outputParquetFile) throws IOException {
		convertCsvToParquet(schemaInfo, csvFile, outputParquetFile, false);
	}

	public static void convertCsvToParquet(Map<String, String> schemaInfo,
			File csvFile, File outputParquetFile, boolean enableDictionary)
			throws IOException {
	...
	private static void closeQuietly(Closeable res) {
		try {
			...
			LOG.warn("Exception closing reader " + res + ": "
					+ ioe.getMessage());
		}
	}
}